...
int main(void)
{
    ...
    ftpClient->ftpClientGet("/sdcard/test.txt", "test.txt", FTP_CLIENT_BINARY, ftpClientNetBuf);
...
static int xfer(const char *localfile, const char *path, NetBuf_t *nControl, int typ, int mode);
...
static int getFileSizeFtpClient(const char *path, unsigned int *size, char mode, NetBuf_t *nControl);
static int getModDateFtpClient(const char *path, char *dt, int max, NetBuf_t *nControl);
...
static int nlstFtpClient(const char *outputfile, const char *path, NetBuf_t *nControl);
static int mlsdFtpClient(const char *outputfile, const char *path, NetBuf_t *nControl);
static int changeDirUpFtpClient(NetBuf_t *nControl);
static int pwdFtpClient(char *path, int max, NetBuf_t *nControl);
/*File to File Transfer*/
static int getDataFtpClient(const char *outputfile, const char *path, char mode, NetBuf_t *nControl);
static int putDataFtpClient(const char *inputfile, const char *path, char mode, NetBuf_t *nControl);
...
static int accessFtpClient(const char *path, int typ, int mode, NetBuf_t *nControl, NetBuf_t **nData);
...
static int socketWait(NetBuf_t *ctl)
{
    ...
    if ((ctl->dir == FTP_CLIENT_CONTROL) || (ctl->idlecb == NULL)) return 1;
...
static int readLine(char* buffer, int max, NetBuf_t* ctl)
{
    if ((ctl->dir != FTP_CLIENT_CONTROL) && (ctl->dir != FTP_CLIENT_READ)) return -1;
    if (max == 0) return 0;
    ...
    while (1) {
        ...
        if (!socketWait(ctl)) return retval;
        if ((x = recv(ctl->handle, ctl->cput,ctl->cleft, 0)) < 0) {
#ifdef FTP_CLIENT_DEBUG
            print_msg(1, TAGFTP, "[%s] FTP Client Error: recv() error\n", __func__);
#endif
            retval = -1;
            break;
        }
...
char match[5];
    ...
    if (readLine(nControl->response, FTP_CLIENT_RESPONSE_BUFFER_SIZE, nControl) < 0) {
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client Error: readLine() failed\n", __func__);
#endif
        return 0;
    }
#ifdef FTP_CLIENT_DEBUG
    print_msg(1, TAGFTP, "[%s] FTP Client response : %s\n", __func__, nControl->response);
#endif
...
#ifdef FTP_CLIENT_DEBUG
    print_msg(1, TAGFTP, "[%s] FTP Client cmd : %s\n", __func__, cmd);
#endif
    if ((strlen(cmd) + 3) > sizeof(buf)) return 0;
    sprintf(buf, "%s\r\n", cmd);
    if (send(nControl->handle, buf, strlen(buf), 0) <= 0) {
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client send : send() error\n", __func__);
#endif
        return 0;
    }
...
NetBuf_t *nData;
    ...
    if (localfile != NULL) {
        char ac[4] = {0};
        if (typ == FTP_CLIENT_FILE_WRITE) ac[0] = 'r';
                                     else ac[0] = 'w';
        if (mode == FTP_CLIENT_IMAGE) ac[1] = 'b';
    ...
    if (!local) local = (typ == FTP_CLIENT_FILE_WRITE) ? stdin : stdout;
    if (!accessFtpClient(path, typ, mode, nControl, &nData)) {
        if (localfile) {
            fclose(local);
            if (typ == FTP_CLIENT_FILE_READ) unlink(localfile);
        }
        return 0;
    }
    ...
    if (typ == FTP_CLIENT_FILE_WRITE) {
        while ((l = fread(dbuf, 1, FTP_CLIENT_BUFFER_SIZE, local)) > 0) {
            ...
            if (c < l) {
                print_msg(1, TAGFTP, "[%s] FTP Client xfer short write: passed %d, wrote %d bytes\n", __func__, l, c);
                rv = 0;
                break;
            }
        }
...
#ifdef FTP_CLIENT_DEBUG
                print_msg(1, TAGFTP, "[%s] FTP Client xfer write to '%s' error\n", __func__, localfile);
#endif
                rv = 0;
                break;
            }
...
static int openPort(NetBuf_t *nControl, NetBuf_t **nData, int mode, int dir)
{
    ...
    if (nControl->cmode == FTP_CLIENT_PASSIVE) {
        ...
        if (!sendCommand("PASV", '2', nControl)) return -1;
...
#ifdef FTP_CLIENT_DEBUG
            print_msg(1, TAGFTP, "[%s] FTP Client error : getsockname()\n", __func__);
#endif
            return -1;
        }
    }
    ...
    if (sData == -1) {
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client error : socket()\n", __func__);
#endif
        return -1;
    }
    if (nControl->cmode == FTP_CLIENT_PASSIVE) {
...
#ifdef FTP_CLIENT_DEBUG
            print_msg(1, TAGFTP, "[%s] FTP Client error : connect()\n", __func__);
#endif
            closesocket(sData);
            return -1;
        }
        ...
        sin.in.sin_port = 0;
        if (bind(sData, &sin.sa, sizeof(sin)) < 0) {
#ifdef FTP_CLIENT_DEBUG
            print_msg(1, TAGFTP, "[%s] FTP Client error : bind()\n", __func__);
#endif
            closesocket(sData);
            return -1;
        }
        if (listen(sData, 1) < 0) {
#ifdef FTP_CLIENT_DEBUG
            print_msg(1, TAGFTP, "[%s] FTP Client error : listen()\n", __func__);
#endif
            closesocket(sData);
            return -1;
        }
        if (getsockname(sData, &sin.sa, &l) < 0) return -1;
        char buf[FTP_CLIENT_TEMP_BUFFER_SIZE];
    ...
    if (!ctrl) {
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client error : ctrl=calloc()\n", __func__);
#endif
        closesocket(sData);
        return -1;
    }
...
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client error : ctrl->buf=malloc()\n", __func__);
#endif
        closesocket(sData);
        free(ctrl);
        return -1;
...
static int writeLine(const char *buf, int len, NetBuf_t *nData)
{
    if (nData->dir != FTP_CLIENT_WRITE) return -1;
    ...
    for (x = 0; x < len; x++) {
        if ((*ubp == '\n') && (lc != '\r')) {
            if (nb == FTP_CLIENT_BUFFER_SIZE) {
                if (!socketWait(nData)) return x;
                w = send(nData->handle, nbp, FTP_CLIENT_BUFFER_SIZE, 0);
                if (w != FTP_CLIENT_BUFFER_SIZE) {
#ifdef FTP_CLIENT_DEBUG
                    print_msg(1, TAGFTP, "[%s] FTP Client send() : net_write1 returned %d, errno = %d\n", __func__, w, errno);
#endif
                    return (-1);
                }
                nb = 0;
        ...
        if (nb == FTP_CLIENT_BUFFER_SIZE) {
            ...
            if (w != FTP_CLIENT_BUFFER_SIZE) {
#ifdef FTP_CLIENT_DEBUG
                print_msg(1, TAGFTP, "[%s] FTP Client send() : net_write2 returned %d, errno = %d\n", __func__, w, errno);
#endif
                return (-1);
            }
            nb = 0;
    ...
    if (nb){
        ...
        if (w != nb) {
#ifdef FTP_CLIENT_DEBUG
            print_msg(1, TAGFTP, "[%s] FTP Client send() : net_write3 returned %d, errno = %d\n", __func__, w, errno);
#endif
            return (-1);
        }
    }
...
fd_set mask;
    ...
    if (i == -1) {
        ...
        strcpy(nControl->response, "FTP Client accept connection timed out waiting for connection");
        closesocket(nData->handle);
        nData->handle = 0;
        rv = 0;
    } else {
...
char buf[FTP_CLIENT_TEMP_BUFFER_SIZE];
    ...
    if (!sendCommand(buf, '2', nControl)) return 0;
                                     else return 1;
}
/*
...
static char *getLastResponseFtpClient(NetBuf_t *nControl)
{
    if ((nControl) && (nControl->dir == FTP_CLIENT_CONTROL)) return nControl->response;
                                                        else return NULL;
}
/*
...
static int getSysTypeFtpClient(char *buf, int max, NetBuf_t *nControl)
{
    if (!sendCommand("SYST", '2', nControl)) return 0;
...
char cmd[FTP_CLIENT_TEMP_BUFFER_SIZE];
    ...
    if (!sendCommand(cmd, '2', nControl)) return 0;
    int rv = 1;
    sprintf(cmd,"SIZE %s", path);
    if(!sendCommand(cmd, '2', nControl)) rv = 0;
    else {
        int resp;
        unsigned int sz;
        if (sscanf(nControl->response, "%d %u", &resp, &sz) == 2) *size = sz;
                                                             else rv = 0;
    }
...
char buf[FTP_CLIENT_TEMP_BUFFER_SIZE];
    ...
    int rv = 1;
    if (!sendCommand(buf, '2', nControl)) rv = 0;
...
#ifdef FTP_CLIENT_DEBUG
    print_msg(1, TAGFTP, "[%s] Try connect to FTP server %s:%u\n", __func__, host, port);
#endif

    int sControl = socket(AF_INET, SOCK_STREAM, 6/*IPPROTO_TCP*/);
    if (sControl < 0) {
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client Error: socket() error(%d)\n", __func__, sControl);
#endif
        return 0;
    }

    struct sockaddr_in sim;
    socklen_t srvlen = sizeof(struct sockaddr);
    memset(&sim, 0, srvlen);
    sim.sin_family = AF_INET;
    sim.sin_port = htons(port);
    sim.sin_addr.s_addr = inet_addr(host);

    if (connect(sControl, (struct sockaddr *)&sim, srvlen) < 0) {
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client Error: connect() error\n", __func__);
#endif
        closesocket(sControl);
        return 0;
    }
    //fcntl(sControl, F_SETFL, (fcntl(sControl, F_GETFL)) | O_NONBLOCK);
    NetBuf_t *ctrl = (NetBuf_t *)calloc(1, sizeof(NetBuf_t));
    if (!ctrl) {
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client Error: calloc() ctrl error\n", __func__);
#endif
        closesocket(sControl);
        return 0;
    }
    ctrl->buf = (char *)malloc(FTP_CLIENT_BUFFER_SIZE);
    if (!ctrl->buf) {
#ifdef FTP_CLIENT_DEBUG
        print_msg(1, TAGFTP, "[%s] FTP Client Error: malloc() ctrl->buf error\n", __func__);
#endif
        closesocket(sControl);
        free(ctrl);
        return 0;
...
#ifdef FTP_CLIENT_DEBUG
    print_msg(1, TAGFTP, "[%s] Connect to FTP server %s:%u OK !\n", __func__, host, port);
#endif
...
{
    if (!nControl) return;
    if (nControl->dir != FTP_CLIENT_CONTROL) return;
    sendCommand("QUIT", '2', nControl);
    if (nControl->handle > 0) closesocket(nControl->handle);
...
static int changeDirUpFtpClient(NetBuf_t *nControl)
{
    if (!sendCommand("CDUP", '2', nControl)) return 0;
                                        else return 1;
}
/*
...
static int pwdFtpClient(char *path, int max, NetBuf_t *nControl)
{
    if (!sendCommand("PWD",'2',nControl)) return 0;
...
char cmd[FTP_CLIENT_TEMP_BUFFER_SIZE];
    ...
    if (!sendCommand(cmd, '3', nControl)) return 0;
...
static int accessFtpClient(const char *path, int typ, int mode, NetBuf_t *nControl, NetBuf_t **nData)
{
    ...
    if (path != NULL) {
        int i = strlen(buf);
        buf[i++] = ' ';
        if ((strlen(path) + i + 1) >= sizeof(buf)) return 0;
        strcpy(&buf[i], path);
    }

    if (openPort(nControl, nData, mode, dir) == -1) return 0;
...
static int readFtpClient(void *buf, int max, NetBuf_t *nData)
{
    if (nData->dir != FTP_CLIENT_READ) return 0;
    ...
    if (nData->idlecb && nData->cbbytes) {
        ...
        if (nData->xfered1 > nData->cbbytes) {
            if (nData->idlecb(nData, nData->xfered, nData->idlearg) == 0) return 0;
            nData->xfered1 = 0;
        }
    }
...
static int writeFtpClient(const void *buf, int len, NetBuf_t *nData)
{
    ...
    int i = 0;
    if (nData->buf) i = writeLine(buf, len, nData);
    else {
        socketWait(nData);
...
static int closeFtpClient(NetBuf_t* nData)
{
    switch (nData->dir) {
        ...
        case FTP_CLIENT_READ:
            ...
            free(nData);
            ctrl->data = NULL;
            if (ctrl && ctrl->response[0] != '4' && ctrl->response[0] != '5') return(readResponse('2', ctrl));
            return 1;
